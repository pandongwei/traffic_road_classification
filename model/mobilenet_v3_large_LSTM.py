...
from keras.layers import CuDNNLSTM,TimeDistributed,Add


class MobileNetV3_Large_LSTM(MobileNetBase):
    def __init__(self, shape, n_class, alpha=1.0, include_top=True):
        ...
        # Returns
            MobileNetv3 model.
        """
        super(MobileNetV3_Large_LSTM, self).__init__(shape, n_class, alpha)
    ...
    def build(self, plot=False):
        ...
        #add up LSTM
        encoded_rows = TimeDistributed(CuDNNLSTM(320))(tmp)
        encoded_columns = CuDNNLSTM(960)(encoded_rows)
        encoded_columns = Reshape((1, 1, 960))(encoded_columns)
        x = Add()([x,encoded_columns])
