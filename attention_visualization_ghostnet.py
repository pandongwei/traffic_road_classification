...
class ModelOutputs():
    ...
    def __call__(self, x):
        ...
        #print(output.shape)
        output = model.squeeze(output)  #这里要根据具体网络进行修改
        output = output.view(output.size(0), -1) #(1,25088)
        #print(output.shape)
...
def show_cam_on_image(img, mask,i):
    ...
    cv2.imwrite("attention_map/ghostnet/{}.jpg".format(i), np.uint8(255 * cam))
...
if __name__ == '__main__':
    ...
    #model = mobilenetv2(pretrained=True, n_class=4)
    model = ghost_net(num_classes=4)
    model = model.cuda()

    # load the coresponding weights
    ...
    pre_weights = '/home/pan/master-thesis-in-mrt/MobileNetV2/logs/fit/20200309-115646/model_final_89.14%.pkl'


    weights = torch.load(pre_weights)
    model.load_state_dict(weights)
    grad_cam = GradCam(model=model,target_layer_names=[str(16)], use_cuda=True)
    ...
    for i, image_path in enumerate(sorted(os.listdir(images_path))):
        ...
        # image_path = '/home/pan/master-thesis-in-mrt/MobileNetV2/hard_images/bicycle_2.png'
            ...
            # show the result and also imwrite the result
